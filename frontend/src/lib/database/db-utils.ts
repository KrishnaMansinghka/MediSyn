...
const DEFAULT_DB: MediSynDatabase = {
  ...
  doctors: [
    {
      ...
      email: "sarah.johnson@downtownmedical.com",
    ...
    },
    {
      ...
      email: "michael.chen@familyhealth.com",
      password: "password123",
  ...
  patients: [
    ...
    },
    {
      ...
      updatedAt: "2025-09-27T14:20:00.000Z",
      isActive: true,
      role: "patient"
    }
  ],
...
export class MediSynDB {
  ...
  async findUserByEmail(email: string): Promise<User | null> {
    ...
    const doctor = db.doctors.find(doc => doc.email === email && doc.isActive);
    if (doctor) return doctor;
    
    const patient = db.patients.find(pat => pat.email === email && pat.isActive);
  ...
  async findUserById(id: string): Promise<User | null> {
    ...
    if (id.startsWith('doc_')) {
      return db.doctors.find(doc => doc.id === id && doc.isActive) || null;
    }
    
    if (id.startsWith('pat_')) {
      return db.patients.find(pat => pat.id === id && pat.isActive) || null;
    }
